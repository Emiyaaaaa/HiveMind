...
**最终对外产物必须干净**，不是「自己别主动写」。本环境跑 `git commit` 会自动注入 `Co-authored-by: Cursor <cursoragent@cursor.com>`；soft reset 后再 `git commit` 仍会注入。

因此：**禁止 `git commit`**。只用 `scripts/commit-index.sh`（内部 `git commit-tree` + `git update-ref`）。写完立刻 `git log -1 --format=%B`；trailer 里还有 cursor 就当失败，不要继续。

Commit subject/body、分支名 **都不能带有任何 cursor 相关的字样**（大小写均禁），包括：`Cursor`、`cursor`、`.cursor`、`cursor.com`、Cursor Agent、Made with Cursor、Generated by Cursor、`Co-authored-by: Cursor`。
...
- [ ] 3. 根据暂存区内容生成分支名并切分支
- [ ] 4. git add .
- [ ] 5. 根据暂存区内容生成 commit message，用 `commit-index.sh`（commit-tree）提交
- [ ] 6. `git log -1 --format=%B` 确认无 cursor trailer
- [ ] 7. 把分支名和 commit 回给用户
```
...
禁止：`git commit`（会注入 Cursor trailer）、改 git config、`--force`、`--amend`、interactive rebase、push、`gh pr create`、reset 默认分支。
...
- message 本身就不许含 `cursor`。不要自己加 `Co-authored-by`。

**不要** `git commit` / `git commit -m` / `--amend`。用脚本（stdin = message）：

```bash
.cursor/skills/create-branch/scripts/commit-index.sh <<'EOF'
feat: add create-branch skill for staged work

EOF
```

脚本会 `git write-tree` + `git commit-tree` + `git update-ref`，并在提交后检查 `%B`。失败则停下。

### 6. 自检

再跑一次 `git log -1 --format=%B`。看到 `Co-authored-by`、`Cursor`、`cursor.com` 或任何 cursor 字样 → 失败，不要把 SHA 当成功。不要再用 `git commit` 重做。
