# Hivemind AutoGen adapter

This optional plugin runs an existing AutoGen AgentChat agent or team and
translates its stream, usage, and tool events to Hivemind's shared runtime
contract.
...
It does not add AutoGen to Hivemind's core dependencies.
...
Hivemind adapters. When configured, the adapter exposes resolved tools to
...
One single-agent run maps to one Hivemind step. Team runs can emit one step per
...
and Hivemind-owned ToolCall IDs for native AutoGen tool events.
